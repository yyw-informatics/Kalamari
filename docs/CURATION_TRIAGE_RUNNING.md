...
8. [CI](#8-ci)
9. [Tool behaviour a naive integration gets wrong](#9-tool-behaviour-a-naive-integration-gets-wrong)
10. [What is run for real, and what is only wired](#10-what-is-run-for-real-and-what-is-only-wired)
11. [Files](#11-files)
...
time, write the directories into `.taxdump-search` at the repository root, one per line,
with `#` comments allowed. The first one holding `nodes.dmp` is used.

That file is **gitignored on purpose.** An absolute path in a committed file would
publish one institution's filesystem layout, and it would be wrong on every other
machine. So the repository ships no default search path at all.
`$NCBI_TAXDUMP_SEARCH` does the same job for CI and one-off shells, separated like
`$PATH`. With none of them set, the resolver falls back to the bundled partial dump and
says so.
...
that owned this pick never reported".

Work through what a wrong count does. A count of 1 against an 8-shard matrix reads only
`leads.0.ndjson`. It finds no file missing, so it concludes the run was complete, and
marks every pick the other seven shards held as `resolved`. Those are false records,
appended to an append-only ledger. So the count is passed explicitly everywhere, and it
is cross-checked when the fragments are named individually.
...
- *Re-run the pipeline at tag time.* It would diff this month's leads against a ledger
  that already contains them. Every lead would come back `recurring`, and the release
...
135 000 records; asking with `--reference` returns one.

So the 151 tracked-species taxids that feed J1b are queried **reference-only**. J1b fires
only for NCBI-designated reference genomes, so that is exactly the question it asks. The
19 wishlist taxids that feed J2 need the full list and get it — 3059 records between
them. The cache records the mode per taxid, so a reference-only entry cannot silently
serve a full-query need.
...
- **A correct name is the status field's last clause, never the substring `correct`.**
  The substring also matches `...inappropriate correction; misspelling`, which covers 12
  rows, 10 of them species. A substring test would therefore let the feeder propose a
  misspelling as a species worth adding.

**What limits it.** The date is an *authority year*, not a full date, and it is derived
here rather than published by LPSN. A name with an unusual authority string therefore
gets no year and sorts last.

The cap, not any cutoff, is what keeps the worklist short. Kalamari lacks about **4000**
validly-published species in the genera it tracks, so `--max-lpsn` (25 by default) is the
real gate. Gaps are sorted newest first, so the cap keeps the most recently named ones.
...
- **The datasets refresh runs once, unsharded, and unwindowed.** The rebuild replaces the
  file rather than merging into it, so shards would clobber each other, and a window
  would leave a cache the monthly job cannot use (§3).
...
Writing over the real file is what arms the guard. Staging through a scratch path leaves
it with nothing to compare against, so it can only warn.
...
[`tool_pins.tsv`](../src/curation-triage/tool_pins.tsv) is the index. It has one row per
...
A binary can also run but print **no version-looking token at all**. That is the ordinary
shape of a broken conda install, for example `error while loading shared libraries:
libgomp.so.1`. It is reported by name as `reports '<nothing>' but the pin is '0.3.2'`,
and the loop carries on to the next environment.

The hazard behind that is worth respecting when editing the script. Under
`set -o pipefail` an empty `grep` exits 1. That status escapes through the assignment, and
`errexit` then kills the whole loop mid-run — no message, no summary. Three places have
this shape:

- a missing version token,
- an AMRFinderPlus build that prints no database-version line,
- an unreadable lockfile.

All three must land in the failure summary instead of ending the run silently.
...
AMRFinderPlus is the only tool here with a first-class dated database version. That is
exactly why it is the *C. botulinum* toxin caller: a changed call is attributable to a
database update rather than to biology.

But the database lives **outside** the conda package. The lockfile installs the software
and nothing else, and `amrfinder -u` downloads the **latest** database, which is not
necessarily the pinned `2026-05-15.1`.
...
- **A committed digest manifest.**
  ...
  holds one SHA-256 per file for all **162 files (239.4 MB)**. It also holds a single
  roll-up, `2fb16b7f3c57…`, over the sorted (name, hash) pairs. That is 20 KB of evidence
  in the repository; the 239.4 MB stays out of it. The roll-up is defined so that plain
...
- **The gate checks it too.** `tool_pins.tsv` carries a `database_digest` column, and the
  ...
  database swapped under an unchanged version string is a gate failure, not only a
...
**Point 1** recomputes every lockfile's sha256 from disk and compares it against
`tool_pins.tsv`. It also checks that the dated AMRFinderPlus version matches
`marker_manifest.tsv`, and that no call depends on a run-time-download database.

**Point 2** replays the committed Tier-2 evidence — 41 units, 75 calls — through the same
functions the monthly run uses, and digests the result to `panel-replay:<12 hex>`. It
*also* checks that the caches' own recorded `skani_version`, `tool_versions` and
`amrfinder_database_version` equal what `tool_pins.tsv` pins.

**Point 3** attributes every moved call to `software`, `database`, `threshold`, `rule` or
`panel`, by asking which of *that call's* components moved.

**Point 4** checks that every component's current identity equals the last `to` in
`pin_changelog.tsv`, and that each component's rows form an unbroken chain.
...
them. A panel replayed from the previous pin's evidence signs off on nothing.

The one recorded exception is handled explicitly. A tool whose `version_command` is
`none` — that is `shigeifinder` — may only appear as `@unknown`. A real version string
there is rejected as evidence from somewhere else.

**The panel's scope is frozen in the gate, not read from the ledger.** Suppose the
trigger set came from the ledger and the policy table. A curator resolving a Tier-1 lead
would then drop a unit, shrink the panel from 75 calls to 74, move the digest and fail
point 2. That is ordinary curation breaking the gate, with no legal changelog row to add.

So the scope is frozen as `PANEL_SCOPE`: 21 confounded units plus 20 Tier-1 lead units,
41 in all. It is a pinned component in its own right, which means **changing what the
gate replays is a recorded decision**. It is written as a `panel` row whose `from` and
`to` are the two scope identities.

A call that appeared or disappeared while the scope moved classifies as `panel`. A call
present on *both* sides during a scope change is still `unattributable`, so the new class
cannot become a blanket excuse.

The trade is deliberate and worth knowing: new Tier-1 leads and new confounded units do
**not** enter the panel on their own. A gate whose evidence can move underneath it is not
a gate.
...
last identity each component was recorded at**: the last `to` in `pin_changelog.tsv`, not
the `from` of that row.

Reading `from` there would make a legitimate skani bump report "pinned components moved:
0", and then fail the very call it has just explained, as unattributable. A component
whose last row records `to = '-'` is treated as removed and drops out.
...
two separate jobs on push and pull request. They are separate because they ask different
...
GitHub Release. Run artifacts expire after 90 days, and a curator looking for a worklist
a year later needs it to still be there.

The release step passes `files` only, never `generate_release_notes`, so it cannot
rewrite the notes upstream's own release job authors. A tag runs *only* that job, and the
worklist is rendered from the ledger (§2).
...
work around.
→ [skani_runner.py:43](../bin/curation-triage/skani_runner.py#L43)

**ShigEiFinder needs its `--tmpdir` to exist already.** Otherwise it dies *after* writing
its header, leaving a file that looks like a clean "no results". Its default `--tmpdir`
is also a relative path.
→ [typers.py:595](../bin/curation-triage/typers.py#L595)

**abricate and mlst need their environment's `bin` first on `PATH`.** Both are Perl
scripts, and off `PATH` they die in a way that reads as a broken install (§5). LisSero
needs the same, for `makeblastdb`.
...
genomospecies (for example *mosaicus*) and the familiar name is in `subspecies(ANI)`. It
does **not** report `rpoB`, so `rpoB` is not in its marker list even though it is a
published SNP discriminator. It writes to a file, not to stdout.
...
LisSero reports `FULL`/`PARTIAL`/`NONE`; ShigEiFinder has no organism column and its
`ipaH` is a bare `+`/`-`. Four of the nine tools write files rather than stdout, each
with its own path convention.
→ [typers.py](../bin/curation-triage/typers.py)

**Two environments need `setuptools<81` added**, because modern setuptools removed
`pkg_resources`.
→ the lockfiles
...
| Two environments recreated **from the committed lockfiles** | `datasets` and `skani` installed into a scratch prefix, reporting their pinned versions |
...
| Sharded refresh, end to end | 3 typer shards plus the skani shard, merged; the result is **byte-identical** to the committed caches |
| The merge path the workflow takes | the typer cache split into the 9 fragments the matrix produces, merged with `--base`: 38 in, 38 out, byte-identical, gate still green |
| The merge's refusals | a partial fragment set, a conflicting value, and the same merge **without** `--base` all exit 1, each naming what it refused |
| The windowed-cache gate | two narrowed datasets caches both make an unwindowed `--offline` run exit 1 naming the gap; the committed cache still passes |
| `setup_envs.sh` failure paths | unknown `--only`, missing environment, tampered digest, database drift with and without `--allow-db-drift`, and a binary with no version token |
| The update gate | `--check` green with no `CONDA_PREFIX`, a bare `PATH`, and both proxies pointed at a dead port; failure forced for all four points |
| Both tiers offline | 8 Tier-1 shards plus collate, then the Tier-2 replay that produced the committed ledger |
| **Two more bot months, simulated** | the pipeline re-run twice on a scratch copy: every lead returns `recurring`, nothing resolves, no epoch or fingerprint moves, gate still passes |
| A failed shard | deleting one of the 8 fragments makes `collate.py` exit 1 with `shards_failed=1/8`, append nothing and write no worklist |
| The workflows' own step scripts | five extracted from the parsed YAML and run locally; every `run:` block in the three fork-owned files passes `bash -n` |
...
| The `amrfinder -u` branch of `setup_envs.sh` | running it destroys the pinned `2026-05-15.1` — the state the pin exists to protect |
| 6 of the 9 typer shards | minutes of compute each, and they change no committed cache; the three cheapest plus skani prove the fragment and merge path |
| The ECTyper shard | needs the 943 MiB Zenodo sketch, deliberately not provisioned; planning it stays behind `--with-ectyper` |
...
| The release-asset job | fires only on a `v*.*.*` tag; only the upload is untested — the renderer it calls was run for real |

The first row is the honest headline: **no workflow has ever run on a real GitHub
runner.** It is listed with the other gaps in the
[design's limits](CURATION_TRIAGE_DESIGN.md#10-honest-limits).
...
| [`pin_changelog.tsv`](../src/curation-triage/pin_changelog.tsv) | every identity each pinned component has ever had: 38 baseline rows plus the AMRFinderPlus database-freeze row |
